...
describe('PaymentService', () => {
  ...
  // REVERSED (session 4, PAY-104): APPROVED -> REVERSED is the only valid
  // route into REVERSED; REVERSED is terminal; repeating REVERSED is
  // idempotent.
  describe('REVERSED (PAY-104)', () => {
    it('allows APPROVED -> REVERSED', () => {
      const service = new PaymentService();
      service.createPayment('pay-1');
      service.applyProviderUpdate('pay-1', 'APPROVED');
      const reversed = service.applyProviderUpdate('pay-1', 'REVERSED');
      expect(reversed.status).toBe('REVERSED');
    });

    it('rejects PENDING -> REVERSED and leaves the stored status untouched', () => {
      const service = new PaymentService();
      service.createPayment('pay-1');
    ...
    it('rejects any further update once a payment is REVERSED (terminal)', () => {
      const service = new PaymentService();
      service.createPayment('pay-1');
      service.applyProviderUpdate('pay-1', 'APPROVED');
      service.applyProviderUpdate('pay-1', 'REVERSED');
