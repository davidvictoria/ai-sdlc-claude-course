...
1. **El diff completo** con `git diff main -- sesiones/sesion-1`. Son tres
   ...
   archivos? ¿Por qué?
2. **El plan aprobado (checkpoint 1).** Pide a un equipo que muestre su
   ...
   `PaymentStatus`. Un equipo que aprobó sin corregir nada probablemente no
   leyó el plan completo.
3. **El lugar del cambio (checkpoint 2).** Contrasta con equipos cuyo diff
...
4. **La evidencia (checkpoint 3).** Muestra la salida real de
   ...
   y pregunta qué equipo aceptó un "ya pasa" de Claude sin ejecutar el
   comando.
